# Welcome to the world of TypeScript!

```
Thanks for checking out the first few parts of the interactive book I am building. I am releasing the first few Modules just to get some feedback. The book will have 2 parts. An interactive app, and a non-interactive portion. This is the initial non-interactive portion. I needed this to start building the interactive app. I am open to any suggestions/fixes, please open an issue or pr. Thanks. - Jett
```

In this crash course, we will embark on a journey to learn and master the basics of programming in TypeScript. There are 24 modules, and at the start of each module, you should start a timer. It is absolutely ok to take more time than 1 hour, do not rush! However, if you have completed a module with time remaining, take the remaining time to review the module before the time runs out, or repeat the exercises. This will help reenforce and retain the knowledge in the module. While this could be considered a beginner course, it by no means covers every aspect of TypeScript. We do assume you have at a minimum very basic knowledge of programming, or have tried to learn programming before. Ready to go? Let's not waste more time. Grab a cup of tea ☕ and [CLICK HERE TO GET STARTED](module_1/1.1_introduction_to_typescript.md)

# Cheat Sheet

Know TypeScript but need a quick reference? Check out the Cheat Sheet.

* [Cheat Sheet](typescript_cheat_sheet.md)

# Modules

1. Introduction to TypeScript
    1. [Introduction to TypeScript](module_1/1.1_introduction_to_typescript.md)
    2. [Installing TypeScript](module_1/1.2_installing_typescript.md)
    3. [Setting up a TypeScript project](module_1/1.3_setting_up_a_typscript_project.md)
    4. [Basic types and variables](module_1/1.4_basic_types_and_variables.md)
    5. [Your first TypeScript Program](module_1/1.5_your_first_typescript_program.md)
    6. [Module 1 Wrap Up](module_1/1.6_module_1_wrap_up.md)

2. TypeScript Functions
    1. [Declaring functions](module_2/2.1_declaring_functions.md)
    2. [Function parameters and return types](module_2/2.2_function_parameters_and_return_types.md)
    3. [Optional and default parameters](module_2/2.3_optional_and_default_parameters.md)
    4. [Function overloading](module_2/2.4_function_overloading.md)
    5. [Module 2 Project](module_2/2.5_module_2_project.md)
    6. [Module 2 Wrap Up](module_2/2.6_module_2_wrap_up.md)

3. TypeScript Classes
    1. Creating classes
    2. Class properties and methods
    3. Constructors and initialization
    4. Access modifiers
    5. Module 3 Project
    6. Module 3 Wrap Up

4. Inheritance and Interfaces
    1. Inheritance and extending classes
    2. Implementing interfaces
    3. Abstract classes
    4. Multiple inheritance using interfaces
    5. Module 4 Project
    6. Module 4 Wrap Up

5. TypeScript Modules
    1. Introduction to modules
    2. Importing and exporting modules
    3. Default and named exports
    4. Organizing code with modules
    5. Module 5 Project
    6. Module 6 Wrap Up

6. TypeScript Generics
    1. Introduction to generics
    2. Using generics with functions and classes
    3. Constraints on generics
    4. Generic types and interfaces
    5. Module 6 Project
    6. Module 6 Wrap Up

7. TypeScript Decorators
    1. Introduction to decorators
    2. Class decorators
    3. Method decorators
    4. Property decorators
    5. Module 7 Project
    6. Module 7 Wrap Up

8. Advanced Types
    1. Union and intersection types
    2. Type guards and assertions
    3. Conditional types
    4. Mapped types
    5. Module 8 Project
    6. Module 8 Wrap Up

9. TypeScript and JavaScript
    1. Interoperability between TypeScript and JavaScript
    2. Using JavaScript libraries in TypeScript
    3. Typing existing JavaScript code
    4. Declaration files
    5. Module 9 Project
    6. Module 9 Wrap Up

10. TypeScript and Node.js
    1. Using TypeScript with Node.js
    2. Setting up a Node.js project with TypeScript
    3. Working with modules in Node.js
    4. Debugging TypeScript code in Node.js
    5. Module 10 Project
    6. Module 10 Wrap Up

11. Asynchronous Programming with TypeScript
    1. Callbacks and promises
    2. Working with async/await
    3. Error handling in asynchronous code
    4. Using async generators
    5. Module 11 Project
    6. Module 11 Wrap Up

12. TypeScript and Express.js
    1. Introduction to Express.js
    2. Setting up an Express.js project with TypeScript
    3. Handling requests and responses
    4. Middleware and routing
    5. Module 12 Project
    6. Module 12 Wrap Up

13. Testing TypeScript Applications
    1. Introduction to testing TypeScript code
    2. Setting up testing frameworks
    3. Writing unit tests with TypeScript
    4. Mocking and stubbing dependencies
    5. Module 13 Project
    6. Module 13 Wrap Up

14. TypeScript and React
    1. Introduction to React with TypeScript
    2. Setting up a React project with TypeScript
    3. Working with components and props
    4. Type safety in React components
    5. Module 14 Project
    6. Module 14 Wrap Up

15. State Management with TypeScript
    1. Introduction to state management
    2. Using Redux with TypeScript
    3. Type-safe actions and reducers
    4. React state management with TypeScript
    5. Module 15 Project
    6. Module 15 Wrap Up

16. TypeScript and Angular
    1. Introduction to Angular with TypeScript
    2. Setting up an Angular project with TypeScript
    3. Components, services, and dependency injection
    4. Type safety in Angular applications
    5. Module 16 Project
    6. Module 16 Wrap Up

17. TypeScript and Vue.js
    1. Introduction to Vue.js with TypeScript
    2. Setting up a Vue.js project with TypeScript
    3. Components, props, and slots
    4. Type safety in Vue.js applications
    5. Module 17 Project
    6. Module 17 Wrap Up

18. Error Handling and Debugging
    1. Debugging TypeScript code
    2. Handling runtime errors with try-catch
    3. Error handling strategies
    4. Logging and monitoring in TypeScript applications
    5. Module 18 Project
    6. Module 18 Wrap Up

19. Optimizing TypeScript Applications
    1. Performance optimization techniques
    2. Minification and bundling
    3. Tree-shaking and dead code elimination
    4. Caching and lazy loading
    5. Module 19 Project
    6. Module 19 Wrap Up

20. Working with External APIs
    1. Consuming RESTful APIs
    2. Making HTTP requests with TypeScript
    3. Serialization and deserialization of data
    4. Error handling in API calls
    5. Module 20 Project
    6. Module 20 Wrap Up

21. TypeScript and Database Integration
    1. Working with databases in TypeScript
    2. Connecting to databases
    3. Querying and manipulating data
    4. Object-relational mapping (ORM)
    5. Module 21 Project
    6. Module 21 Wrap Up

22. Internationalization and Localization
    1. Introduction to internationalization and localization
    2. Handling multiple languages in TypeScript
    3. Formatting dates, numbers, and currencies
    4. Translating user interfaces
    5. Module 22 Project
    6. Module 22 Wrap Up

23. TypeScript Best Practices
    1. Writing clean and maintainable code
    2. TypeScript coding conventions
    3. Error handling and defensive programming
    4. Performance optimization tips
    5. Module 23 Project
    6. Module 23 Wrap Up

24. Deploying TypeScript Applications
    1. Building TypeScript applications for production
    2. Deploying to different environments
    3. Continuous integration and deployment (CI/CD)
    4. Monitoring and scaling TypeScript applications
    5. Module 24 Project
    6. Module 24 Wrap Up

25. Bonus Materials
    1. Deploy to Vercel
    2. Deploy to Heroku
    3. Deploy to Azure
    4. Deploy to AWS
